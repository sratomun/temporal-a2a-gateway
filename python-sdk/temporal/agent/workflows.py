...
async def _agent_task_activity(agent_id: str, message_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Find the agent's activity method
    activity_method = None
    for attr_name in dir(agent):
        attr = getattr(agent, attr_name)
        if hasattr(attr, '_is_agent_activity') and not getattr(attr, '_is_streaming', False):
            activity_method = attr
            break
    
    if not activity_method:
        raise ValueError(f"Agent {agent_id} has no @agent_activity method")
    
    # Call the agent's activity
    return await activity_method(message_data)
...
async def _agent_streaming_activity(agent_id: str, message_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Find the agent's streaming activity method
    activity_method = None
    for attr_name in dir(agent):
        attr = getattr(agent, attr_name)
        if hasattr(attr, '_is_agent_activity') and getattr(attr, '_is_streaming', False):
            activity_method = attr
            break
    
    if not activity_method:
        raise ValueError(f"Agent {agent_id} has no streaming @agent_activity method")
    
    # Call the agent's streaming activity
    return await activity_method(message_data)
