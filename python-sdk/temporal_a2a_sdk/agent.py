...
def agent_activity(func):
    ...
    async def wrapper(self, message_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(params) >= 3 and params[2] == 'stream':
            ...
            # For streaming, activity returns None (chunks sent via stream)
            if result is None:
                # Return success indicator for SDK
                return {"is_streaming": True, "status": "completed"}
            else:
                raise ValueError(f"Streaming activity should return None (chunks sent via stream), got {type(result)}")
        else:
            # Simple activity - just pass text
            result = await func(self, text)
            
            # For simple messages, result should be str
            if isinstance(result, str):
                return A2AResponse.text(result, name="Response").to_dict()
            else:
                raise ValueError(f"Message activity must return str, got {type(result)}")
    ...
    # Set a predictable name based on the method name
